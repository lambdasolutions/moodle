...
function kalturaInitModalBox(url, options) {
    //create timestamped unique id for every time the iframe opens
    var starta = new Date();
    // timestamp to id
    tmp_id = starta.getTime();
...
function scrollleft(divid, amount) {
    scroller = document.getElementById(divid);
    if (scroller) {
        if (amount) scroller.scrollLeft -= amount;
...
function switch_item_in_player(entry_id, type) {
    YUI().use('node', 'io',
    function(Y) {
        var static_library_img = Y.one('#static_library_img');
        var static_library_player = Y.one('#static_library_player');
        if (!type || type == 2) {
            ...
            if (!static_library_img) {
                var div = Y.one('#static_library_player_div');
                var img = div.create('<img>');
                img.set('id', 'static_library_img');
                div.appendChild(img);
                static_library_img = img;
            }

            static_library_img.set('src', img_src);
        } else {
            static_library_player.show(true);
            kdp = new KalturaPlayerController('static_library_player');
            kdp.insertEntry(entry_id);
            static_library_img.hide(true);
        }
    })
}
...
function switch_data(url, entry) {
    YUI().use('node', 'io',
    function(Y) {
        var kitem_metadata = Y.one('#kitem_metadata');
        if (!entry) {
            kitem_metadata.load(url);
        } else {
            data = 'entry_id=' + entry;
            data += '&title=' + kitem_metadata.one(':input[name=title]').get('value');
            data += '&desciption=' + kitem_metadata.one(':input[name=description]').get('value');
            data += '&tags=' + kitem_metadata.one(':input[name=tags]').get('value');
            data += '&' + kitem_metadata.one(':input[name=entry_id]').get('value');
            data += '&field=' + kitem_metadata.one(':input[name=field]').get('value');
            data += '&clone=' + kitem_metadata.one(':input[name=clone]').get('value');
            data += '&skippreview=' + kitem_metadata.one(':input[name=skippreview]').get('value');
            data += '&fullinject=' + kitem_metadata.one(':input[name=fullinject]').get('value');
            data += '&presspost=' + kitem_metadata.one(':input[name=presspost]').get('value');
...
function fix_window_size(options) {
    ...
    function(Y) {
        ...
        var objModalbox = Y.one('#modalbox');
        if (objModalbox) {
            objModalbox.setStyles({
                width: width,
                height: height,
            });
            objModalbox.setStyle('margin-top', (0 - height / 2) + 'px');
            objModalbox.setStyle('margin-left', (0 - width / 2) + 'px');
        }
    });
}
...
function openLightBox(entryId, objId) {
    ...
    function(Y) {
        if (this == window.top) {
            ...
            //else
            kdps_ff = Y.NodeList.getDOMNodes(Y.all('div.kaltura_wrapper').all('embed'));
