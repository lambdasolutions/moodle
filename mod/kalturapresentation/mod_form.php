...
/**
 * kalturapresentation configuration form
 *
 * @package    mod
...
class mod_kalturapresentation_mod_form extends moodleform_mod {
    function definition() {
        ...
        $config = get_config('kalturapresentation');
...
INLINE;
        $mform->addElement('static', 'swfdoc_area', '', $interface);


        $mform->addElement('submit', 'syncpoints', get_string('syncpoints', 'kalturapresentation'), array('disabled'));
        ...
        //-------------------------------------------------------
        $mform->addElement('header', 'optionssection', get_string('optionsheader', 'kalturapresentation'));
        ...
        if (count($options) == 1) {
            ...
            $mform->addElement('select', 'display', get_string('displayselect', 'kalturapresentation'), $options);
            $mform->setDefault('display', $config->display);
            $mform->setAdvanced('display', $config->display_adv);
            $mform->addHelpButton('display', 'displayselect', 'kalturapresentation');
        }

        if (array_key_exists(RESOURCELIB_DISPLAY_POPUP, $options)) {
            $mform->addElement('text', 'popupwidth', get_string('popupwidth', 'kalturapresentation'), array('size'=>3));
            if (count($options) > 1) {
            ...
            $mform->addElement('text', 'popupheight', get_string('popupheight', 'kalturapresentation'), array('size'=>3));
            if (count($options) > 1) {
        ...
        if (array_key_exists(RESOURCELIB_DISPLAY_AUTO, $options) or
          ...
          array_key_exists(RESOURCELIB_DISPLAY_FRAME, $options)) {
            $mform->addElement('checkbox', 'printheading', get_string('printheading', 'kalturapresentation'));
            ...
            $mform->addElement('checkbox', 'printintro', get_string('printintro', 'kalturapresentation'));
